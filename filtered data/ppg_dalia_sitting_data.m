%% === AUTO-GENERATE PLOTS FOR ALL SUBJECTS (SITTING ACTIVITY) ===
% Outputs Sitting_Subject1.png ... Sitting_Subject15.png
...
% Sitting activity file
act_file = 'ppg_dalia_sitting_data.mat';   % <-- UPDATED
...
fprintf("Detected %d subjects in Sitting dataset.\n", num_subjects);
...
for subject_idx = 1:num_subjects
    ...
    sgtitle(sprintf('Sitting Activity - Subject %d', subject_idx));

    % === Save figure ===
    save_name = fullfile(save_dir, sprintf('Sitting_Subject%d.png', subject_idx));
    saveas(gcf, save_name);

    close(gcf);
end

fprintf("\nAll Sitting subject plots saved successfully!\n");
fprintf("Location: %s\n", save_dir);
