%% === AUTO-GENERATE PLOTS FOR ALL SUBJECTS (TABLE SOCCER ACTIVITY) ===
% Outputs TableSoccer_Subject1.png ... TableSoccer_Subject15.png
...
% Table soccer activity file
act_file = 'ppg_dalia_table_soccer_data.mat';   % <-- UPDATED

% Load dataset
S = load(fullfile(data_dir, act_file));
data = S.data;

num_subjects = numel(data);   % typically 15
duration_sec = 60;
...
for subject_idx = 1:num_subjects
    ...
    sgtitle(sprintf('Table Soccer Activity - Subject %d', subject_idx));

    % === Save the figure ===
    save_name = fullfile(save_dir, sprintf('TableSoccer_Subject%d.png', subject_idx));
    saveas(gcf, save_name);

    close(gcf);
end

fprintf("\nAll Table Soccer subject plots saved successfully!\n");
fprintf("Location: %s\n", save_dir);
