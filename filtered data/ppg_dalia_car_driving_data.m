%% === AUTO-GENERATE PLOTS FOR ALL SUBJECTS (CAR DRIVING ACTIVITY) ===
% Outputs CarDriving_Subject1.png ... CarDriving_Subject15.png
...
% Car driving activity file
act_file = 'ppg_dalia_car_driving_data.mat';   % <-- UPDATED
...
fprintf("Detected %d subjects in Car Driving dataset.\n", num_subjects);
...
for subject_idx = 1:num_subjects
    ...
    sgtitle(sprintf('Car Driving Activity - Subject %d', subject_idx));

    % === Save the figure ===
    save_name = fullfile(save_dir, sprintf('CarDriving_Subject%d.png', subject_idx));
    saveas(gcf, save_name);

    close(gcf);  % free memory
end

fprintf("\nAll Car Driving subject plots saved successfully!\n");
fprintf("Location: %s\n", save_dir);
